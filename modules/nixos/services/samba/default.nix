...
let
  cfg = config.afxq.services.samba;

  inherit (lib) types mkEnableOption mkIf mapAttrs optionalAttrs;

  inherit (lib.afxq) mkOpt mkBoolOpt;
    ...
    submodule ({ name, ... }: {
      options = {
        ...
        only-owner-editable = mkBoolOpt false
          "Whether the share is only writable by the system owner (afxq.user.name).";
...
in
{
  options.afxq.services.samba = with types; {
        ...
        (name: value:
          {
            ...
            "write list" = config.afxq.user.name;
