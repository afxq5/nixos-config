...
with lib.afxq;
let
  cfg = config.afxq.services.openssh;

  user = config.users.users.${config.afxq.user.name};
  ...
  other-hosts = lib.filterAttrs
    (key: host: key != name && (host.config.afxq.user.name or null) != null)
  ...
  other-hosts-config = lib.concatMapStringsSep "\n"
    (name:
      let
        ...
        remote-user-name = remote.config.afxq.user.name;
        remote-user-id =
...
in
{
  options.afxq.services.openssh = with types; {
  ...
  config = mkIf cfg.enable {
    ...
    afxq.user.extraOptions.openssh.authorizedKeys.keys = cfg.authorizedKeys;
  };
}
